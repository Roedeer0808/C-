...
// 기본 매개 변수를 지정하기 위해서는 오른쪽에서 부터 지정해주어야 합니다.
...
#pragma region 스트림
	...
	// int data = 0;
	// 
	// std::cout << "data 변수의 값을 입력 : ";
	// 
	// std::cin >> data;
	// 
	// std::cout << "data 변수의 값 : " << data << std::endl;

	// 스트림은 운영체제에 의해 생성되며, 스트림 자체에 버러라는
	// 임시 메모리 공간이 존재합니다.
...
#pragma region 동적 할당
	...
	// //내가 만든 코드
	// // int heap = *ptr;
	// // std::cout << "heap 변수의 값을 입력:";
	// // std::cin >> heap;
	// // std::cout << "heap 변수의 값 : " << heap<< std::endl;
	// 
	// // 쌤 코드
	// *ptr = 9999;
	// dynamicArray[0] = 10;
	...
	// room[1] = 20;
	// room[2] = 30;
	// std::cout << "room[0]이 가리키고 있는 메모리 값 : " << room[0] << std::endl;
	// std::cout << "room[1]이 가리키고 있는 메모리 값 : " << room[1] << std::endl;
	// std::cout << "room[2]이 가리키고 있는 메모리 값 : " << room[2] << std::endl;
...
#pragma region 범위 지정 연산자
	...
	// 
	// // 범위 지정 연산자는 전역 변수와 같은 이름의 지역 변수가 선언되었을 때 가장
	// // 가까운 범위에 선언된 변수의 이름을 사용하는 범위 규칙이 
